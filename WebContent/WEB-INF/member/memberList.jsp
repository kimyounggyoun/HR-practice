...
<script type="text/javascript"> 
  ...
  function memberList(){
	$.ajax({
		...
		dataType : "json",
		success : resultHtml,
		error : function() {alert("memberList error");}
	});  
  }
  function resultHtml (data){
	  var html="<table class='table table-hover'>";
			html+="<tr>"
			html+="<th>번호</th><th>아이디</th><th>비밀번호</th><th>이름</th>"
				+"<th>나이</th><th>이메일</th><th>전화번호</th><th>삭제</th>";
			html+="</tr>";
			$.each(data, function(index, obj){
			    html+="<tr>"
			    html+="<td>"+obj.num+"</td><td>"+obj.id+"</td><td>"+obj.pass+"</td><td>"+obj.name+"</td>"
			        +"<td>"+obj.age+"</td><td>"+obj.email+"</td><td>"+obj.phone+"</td>"
			        +"<td><input type='button' value='삭제' class='btn btn-warning' onclick='delFn("+obj.num+")'></td>";
			    html+="</tr>";
			});
			html+="</table>";
  ...
  function delFn (num) {
	  $.ajax({
		 ...
		 data : {"num" : num},
		 success : memberList,
		 error : function() {alert("delFn error");}
	  });
  }
</script>
</head>
    	  ...
    	  <tr>
    	    ...
    	    <td>
    	    	<c:if test="${vo.filename != null && vo.filename != ''}">
    	    		<img src="<c:out value='file_repo/${vo.filename}'/>" width="60px" hight="60px"/>
    	    	</c:if>
